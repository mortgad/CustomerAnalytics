...
## Check if EFA applies

- Here, we inspect the correlation matrix.
- If EFA applies, we should see many variable pairs with high correlation.
- Running the chunk below reveals how x15 does not correlate with other variables. This could be reason for deletion.
- "High Correlation" is a correlation above 0.4.
...
## EFA with Common Factor Analysis

- First, extract as many factors as number of variables and check eigenvalues
- Check for number of eigenvalues above 1.
- Based on the eigenvalues we go for 4 factors and see later if necessary to reduce this number
...
- Apply Factor Analysis using the function factanal() :)))
...
- To ease interpretation, very low loadings (<.1, <.2) are not displayed (see the cutoff arg in the print statement)

According to the guidelines:
- High loadings (>0.6 or > 0.7) are expected for the items 
- Items with high cross-loadings should be removed (It means these items do not measure a single construct)
...
- A significant chi-square test indicates POOR FIT. The null hypothesis is low deviance between actual and fitted matrix.
- But this test is not always reliable because it is highly influenced by the sample size analyzed. 

Below is an example of interpreting a factor analysis.
...
# The chi-square test is highly significant (p-value is 1.83e-19) meaning poor fit
...
# Communalities
apply(fit1$loadings^2,1,sum) # as one can observe, some items like x15 have very low communality
```

## Refine EFA

- Remove two indicators (x15 and x17) in accordance with guidelines
- Interpret the output again
...
Here, we specify a model in accordance with theory.
Notice we do not need to specify the correlations between the exogenous constructs, since they are allowed to correlate in CFA.

NOTE: we get "lavaan WARNING: some estimated ov variances are negative" 
- (ov = observed variable)
- This is called the "Heywood case".
...
# Before removing x11, I ask for the modification indices as a last check
...
- Same model, except removing x11 and not specifying correlations between exogenous constructs (optional)
...
- RMSEA < 0.08
- SRMR < 0.08. 
- Some sources require CFI > 0.95, TLI > 0.95, RMSEA < 0.05, SRMR < 0.05. 

Loadings:
- Examine the loadings significance, size and sign 
- It is desirable to have high and significant loading reflecting items convergent validity. 
- In the output standardized loadings are in the last column "Std.all""

Reliability:
- Reliability = degree of consistency between multiple measurements of a variable 
- use semtools::reliability(fit)
...
- Are the latent variables significantly different? Do they represent different constructs?
- Run semTools::discriminantValidity(fit, merge=TRUE) (https://rdrr.io/cran/semTools/man/discriminantValidity.html)
- Output: 
  ...
  - than the inter-constructs corelations (or AVE > corr^2).
...
Run fitmeasures(fit) for alternative summary
...
Plus extending the model consistent with the theory: 
Sem.model2 proposed x19 (Satisfaction) as mediator between the four latent constructs and Likelihood of future purchase (x21)
